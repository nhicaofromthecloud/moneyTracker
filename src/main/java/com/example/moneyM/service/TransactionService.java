...
public class TransactionService {
	...
	public List<TransactionResponse> getAllTransactions(Long userId, String description) {
		List<Transaction> transactionList = new ArrayList<Transaction>();
		if(userId == null && description == null) {
			transactionList = transactionRepository.findAll();
		} else if(userId != null && description != null) {
			transactionList = transactionRepository.findByUserAccountUserIdAndDescriptionContainingIgnoreCase(userId, description);
		} else if(userId != null){
			transactionList = transactionRepository.findByUserAccountUserId(userId);
		}
	...
	public List<TransactionResponse> getTransactionsByDateRange(Long userId, LocalDate startDate, LocalDate endDate) {
		List<Transaction> transactionList = transactionRepository
				.findAllByUserAccountUserIdAndDateBetween(userId,startDate, endDate);
		return transactionList
				.stream()
				.map(transaction -> modelMapper.map(transaction, TransactionResponse.class))
				.collect(Collectors.toList());
